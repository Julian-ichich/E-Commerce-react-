...
function SignIn() {
  ...
  const {setShowUser} = useContext(ShoppingCartContext)

  const [email, setEmail] = useState('')
  const [password, setPassword] = useState('')
  const flag = localStorage.getItem('Sign-Out')
  ...
  const loginUsuario = () => {
    ...
    UserList.forEach(element => {
      if (element.email === email.trim() && element.password === password.trim()) {
        setShowUser(element.email)
  ...
  const usuarioFinal = JSON.parse(usuarioGuardado)
          ...
          <div className="bg-black    overflow-hidden  ">
            <div id="myDIV" >
              <div className="w-[100vw] h-[100vh] px-3 sm:px-5 flex items-center justify-center absolute flex-col">
                <div className="w-full sm:w-1/2 lg:2/3 px-6 bg-black  bg-clip-padding backdrop-filter backdrop-blur-sm text-white z-50 py-4  rounded-lg">
                  ...
                  <div className="mb-6">
                    ...
                    <input onChange={(e) => setEmail(e.target.value)} value={email} type="email" className="bg-gray-50 border border-gray-300 text-gray-900 text-xs rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-1.5 md:p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="name@neurolink.com" required />
                  </div>
                  <div className="mb-6">
                    <label className="block mb-2 text-xs font-medium text-white">Your password</label>
                    <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="bg-gray-50 border border-gray-300 text-gray-900 text-xs rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-1.5 md:p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required />
                  </div>
                  <div className="flex flex-row justify-between">
                    <div className="text-white text-sm md:text-md ">Forgot Password</div>
                    <div className="text-[#00FF00] text-sm md:text-md">Signip</div>
                  </div>
                  <div onClick={() => loginUsuario()} className="mt-4 md:mt-10 w-full flex justify-center text-sm md:text-xl bg-[#00FF00] py-2 rounded-md text-black">
                    Login
                  </div>

                </div>
                <NavLink to='/sign-up' className="mt-4 md:mt-10 w-60 flex justify-center text-sm md:text-xl bg-[#00FF00] py-2 rounded-md">
                  sign Up
                </NavLink>
          ...
          >
            ...
            <div className="top-16 z-10 flex items-center flex-col gap-4 px-5 py-5">
              ...
              <div className="flex items-center flex-col">
                <p title="name/نام" className="text-black font-Roboto-md">{usuarioFinal.name}</p>
              ...
              <div className="flex items-center gap-3">
                <button onClick={() => logOut()} className="bg-green-400 transition-all gradient text-[15px] text-white px-3 py-[6px] rounded-full flex items-center gap-1">
                  Log-Out
